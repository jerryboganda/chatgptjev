...
test("the markup helper distinguishes ordinary Markdown from interactive markup", () => {
...
test("every text-bearing block is judged, chrome is dropped, and answer content is kept", async () => {
  const jev = withJev({ [prose.text]: 0.9, [list.text]: 0.9, [widget.text]: 0.05, [richCard.text]: 0.9, [plain.text]: 0.9 });
  restore = jev.restore;
  ...
  expect(jev.seen[0]!.state.blocks.map(block => block.text)).toEqual([prose.text, list.text, widget.text, richCard.text, plain.text]);
  expect(Object.keys(jev.seen[0]!.questions)).toEqual(["block_0", "block_1", "block_2", "block_3", "block_4"]);
...
test("verdicts are sticky per block: no re-asking and no retroactive drop after a kept block was committed", async () => {
  const jev = withJev({ [prose.text]: 0.9, [widget.text]: 0.9, [plain.text]: 0.9 });
  restore = jev.restore;
  ...
  // A completed Jev verdict can be reused for identical content.
...
test("Jev failures stop filtering and rejected chrome is never restored", async () => {
  const failing = withJev(() => { throw new Error("gateway down"); });
  restore = failing.restore;
  await expect(new ChatGptWidgetFilter().filter([widget, plain])).rejects.toThrow(/Jev.*failed/);
  restore();

  const disabled = withJev({ [widget.text]: 0 }, false);
  restore = disabled.restore;
  await expect(new ChatGptWidgetFilter().filter([widget, plain])).rejects.toThrow(/Jev.*disabled/);
  expect(disabled.seen).toHaveLength(0);
  restore();

  const allChrome = withJev({ [widget.text]: 0 });
  restore = allChrome.restore;
  expect(await new ChatGptWidgetFilter().filter([widget])).toEqual([]);
});

test("all blocks are judged before release, using bounded batches", async () => {
  ...
  const jev = withJev({ ...Object.fromEntries(many.map(entry => [entry.text, 0])), [plain.text]: 0.9 });
  restore = jev.restore;
  const filter = new ChatGptWidgetFilter();

  const first = await filter.filter([...many, plain]);
  expect(jev.seen[0]!.state.blocks).toHaveLength(MAX_WIDGET_CANDIDATES);
  expect(first.map(entry => entry.key)).toEqual(["4:p"]);

  const second = await filter.filter([...many, plain]);
  expect(jev.seen).toHaveLength(2);
  expect(jev.seen[1]!.state.blocks).toHaveLength(3);
  expect(second.map(entry => entry.key)).toEqual(["4:p"]);
});

test("an uncertain widget verdict is not cached as approved content", async () => {
