...
VERSION="${VERSION#v}"
if [ -z "$VERSION" ]; then
  echo "Could not resolve the latest ChatGPT Jev release" >&2
  exit 1
fi
case "$VERSION" in
...
ASSET="chatgpt-jev-$VERSION-$PLATFORM-$ARCH.$EXTENSION"
BASE_URL="https://github.com/$REPOSITORY/releases/download/v$VERSION"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/chatgpt-jev-launcher.XXXXXX")"
...
if [ "$OS" = "Darwin" ]; then
  ...
  SOURCE_APP="$STAGE_DIR/ChatGPT Jev.app"
  if [ ! -d "$SOURCE_APP" ] || [ ! -x "$SOURCE_APP/Contents/MacOS/ChatGPT Jev" ]; then
  ...
  TARGET_APP="$INSTALL_DIR/ChatGPT Jev.app"
  if pgrep -x "ChatGPT Jev" >/dev/null 2>&1; then
    echo "Quit ChatGPT Jev before updating it" >&2
    exit 1
  fi
  BACKUP_APP="$TEMP_DIR/ChatGPT Jev.previous.app"
...
LIB_DIR="${CODEX_WEB_GPT_LIB_DIR:-$HOME/.local/lib/chatgpt-jev}"
BIN_DIR="${CODEX_WEB_GPT_BIN_DIR:-$HOME/.local/bin}"
TARGET_DIR="$LIB_DIR/$VERSION"
TARGET="$TARGET_DIR/ChatGPT Jev.AppImage"
WRAPPER="$BIN_DIR/chatgpt-jev"
CORE_HOME="${CHATGPT_JEV_HOME:-$HOME/.chatgpt-jev}"
...
if { [ -n "$RUNNING_PID" ] && kill -0 "$RUNNING_PID" 2>/dev/null; } \
  || pgrep -f "ChatGPT Jev\\.AppImage" >/dev/null 2>&1; then
  echo "Quit ChatGPT Jev before updating it" >&2
  exit 1
fi
...
install -m 0644 "$ICON_SOURCE" "$ICON_DIR/chatgpt-jev.png"
DESKTOP_WRAPPER="$(printf '%s' "$WRAPPER" | sed \
  ...
  -e 's/\$/\\$/g' \
  -e 's/%/%%/g')"
cat > "$APPLICATIONS_DIR/chatgpt-jev.desktop" <<EOF
[Desktop Entry]
Type=Application
Version=1.0
Name=ChatGPT Jev
...
StartupWMClass=chatgpt-jev
EOF
chmod 0644 "$APPLICATIONS_DIR/chatgpt-jev.desktop"
