...
const root = join(homedir(), `.chatgpt-jev-release-smoke-${process.pid}-${Date.now()}`);
...
for (const forbidden of [sourceRoot, dirname(sourceBundle), "/private/tmp/chatgpt-jev-verify", "/tmp/chatgpt-jev-verify"]) {
...
const env = { ...process.env, CHATGPT_JEV_HOME: appHome, CODEX_HOME: codexHome };
...
try {
  ...
  if (payload.service !== "chatgpt-jev" || payload.mode !== "browser-only") {
