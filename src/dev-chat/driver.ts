...
export const DEV_CHAT_SYSTEM_INSTRUCTIONS = [
  "You are running inside the ChatGPT Jev DEV outer-harness simulator.",
...
export class DevChatDriver {
  ...
  async send(
    ...
    for (let round = 0; round < 64; round += 1) {
      ...
      const response = await responseRequest(new Request("http://chatgpt-jev.dev/v1/responses", {
        method: "POST",
  ...
  private async compactInput(
    ...
    const response = await compactRequest(new Request("http://chatgpt-jev.dev/v1/responses/compact", {
      method: "POST",
      headers: {
