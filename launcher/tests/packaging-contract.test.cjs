...
test("release installers resolve checksummed native launcher assets", () => {
  ...
  assert.match(shellInstaller, /chatgpt-jev\.desktop/);
...
test("CI packages and smoke-launches on macOS, Windows, and Linux", () => {
  ...
  assert.match(release, /ChatGPT Jev\.app/);
...
test("Linux AppImage fallback uses one owned extraction and removes it on exit", {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "chatgpt-jev-appimage-runner-"));
  const runtime = path.join(root, "runtime");
  const appImage = path.join(root, "ChatGPT Jev.AppImage");
  ...
  const fallbackRoot = path.join(runtime, `chatgpt-jev-appimage-${process.getuid?.() ?? 0}`);
