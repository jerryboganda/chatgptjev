...
test("Jev is required and compatibility calls never change the Codex route or runtime setup", async () => {
  ...
  assert.equal(hostFor({ ...config, jevEnabled: false }).host.jevStatus().enabled, true);
  ...
  await assert.rejects(disabled.host.setJev(false), /Jev is required/);
  assert.equal(disabled.invocation()?.name, undefined);

  const zeroRisk = hostFor(
    { mode: "full", browserHost: "launcher", browserInteractionMode: "manual", autoApproveToolCalls: true },
    "manual",
  );
  assert.equal((await zeroRisk.host.setJev(true)).enabled, true);
  assert.equal(zeroRisk.invocation()?.name, undefined);

  const dev = devHostFor(config);
  assert.equal((await dev.host.setJev(true)).enabled, true);
  assert.equal(dev.invocation()?.name, undefined);
  assert.equal((await hostFor(undefined).host.setJev(true)).configured, false);
});
...
test("failed launcher update restores every mutable setup file before restarting the previous runtime", async () => {
  ...
  const configLinkPath = process.platform === "win32" ? codexHome : codexConfigPath;
  const codexModelsCachePath = path.join(codexHome, "models_cache.json");
  const oldConfig = {
    mode: "full",
    browserHost: "launcher",
  ...
  if (process.platform === "win32") fs.symlinkSync(sharedDirectory, codexHome, "junction");
  ...
  if (process.platform !== "win32") fs.symlinkSync(sharedConfigPath, codexConfigPath);
  const linkTarget = fs.readlinkSync(configLinkPath);
  const linkInode = fs.lstatSync(configLinkPath).ino;
  ...
  try {
    ...
    assert.equal(fs.readFileSync(sharedConfigPath, "utf8"), "old codex config\n");
    assert.equal(fs.lstatSync(configLinkPath).isSymbolicLink(), true);
    assert.equal(fs.lstatSync(configLinkPath).ino, linkInode);
    assert.equal(fs.readlinkSync(configLinkPath), linkTarget);
