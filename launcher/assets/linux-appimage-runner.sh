...
if [ "$#" -lt 1 ]; then
  echo "ChatGPT Jev AppImage runner requires an AppImage path" >&2
  exit 64
fi
APPIMAGE_PATH="$1"
shift
case "$APPIMAGE_PATH" in
  /*) ;;
  *) echo "ChatGPT Jev AppImage path must be absolute" >&2; exit 64 ;;
esac
if [ ! -f "$APPIMAGE_PATH" ] || [ ! -x "$APPIMAGE_PATH" ]; then
  echo "ChatGPT Jev AppImage is unavailable: $APPIMAGE_PATH" >&2
  exit 66
fi
...
FALLBACK_ROOT="$RUNTIME_PARENT/chatgpt-jev-appimage-$(id -u)"
umask 077
...
if [ ! -x "$APP_DIR/AppRun" ]; then
  echo "ChatGPT Jev AppImage fallback extraction produced no executable AppRun" >&2
  exit 70
fi
